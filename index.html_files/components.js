...
$('document').ready(function () {
    $(".marketplacepage").on("click","#play-video",function (e) {
        user_clicked = 1;
        e.preventDefault();
        e.stopPropagation();
        if (player_init == 0) {
...
app.controller('CatalogSearch', ['$scope', 'LoadMoreService', '$window', '$rootScope', '$timeout', '$interval', '$http', 'SearchService', function ($scope, LoadMoreService, $window, $rootScope, $timeout, $interval, $http, SearchService) {
    ...
    $scope.getPromotionProduct = function (discountItem) {
        ...
        $.each(discountItem, function (index, value) {
            tempArray = value.displayProperties.offerIds!= null ? value.displayProperties.offerIds.join().split(","): [];
            //List is empty
            if (list.length === 0) {
                list = tempArray;
            }
    ...
    $scope.getSalePromotionData = function () {
        ...
        if ($rootScope.promotionData !== undefined) {
            ...
            if(discountItem.length > 0){
				 $http.get("/bin/minecraft/productmanagement.saleproducts.json?locale=" + locale + "&id=" + $scope.promotionIDs.splice(0,100).toString()).then(function (response) {
